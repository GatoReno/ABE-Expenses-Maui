...
namespace AbeXP.UseCases
{
    internal class GetTransactionsUseCase : IGetTransactionsUseCase
    {
        ...
        public GetTransactionsUseCase(ITransactionsRepository transactionsRepository, IGetPaymentMethodsUseCase getPaymentMethodsUseCase, IGetTagsUseCase getTagsUseCase, IUserSession userSession, IAnalyticsService analyticsService)
        {
        ...
        public async Task<Result<IEnumerable<TransactionItem>>> ExecuteAsync(TransactionRequest request)
        {
            ...
            {
                await _analyticsService.LogEventAsync("get_transactions_failed", new Dictionary<string, string>
                {
                    ["exception"] = ex.Message
                });

                return Result.Fail<IEnumerable<TransactionItem>>(new ExceptionalError(ex));
            }
        }
    }
}
